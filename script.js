let store = localStorage.getItem('lang');
let eng = store === 'eng';
localStorage.setItem('lang', eng ? 'eng' : 'ru');
...
const renderButtons = () => {
    const selector = `.${eng ? 'eng' : 'ru'}>span`;
...
const onKeyboardEvent = (event) => {
    ...
    document.querySelector(`.${event.code}`).classList.remove('chosen');
};
...
for (let i = 0; i < keyCodes.length; i++) {
    ...
    for (let j = 0; j < keyCodes[i].length; j++) {
        ...
        table[keyCodes[i][j]] = { ru: { low: keysRu[i][j], up: keysRuUp[i][j] }, eng: { low: keysEng[i][j], up: keysEngUp[i][j] }};
...
document.addEventListener('keydown', (event) => {
    ...
    document.querySelector(`.${event.code}`).classList.add('chosen');
});
